...
const Profile = ({ navigation }) => {
  ...
  const fetchEntitlement = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "users", user.uid), { sub: subValue });
    } catch (error) {
  ...
  const deleteAccount = () => {
    ...
    const askPasswordAndReauth = () => {
      Alert.prompt(
        ...
        "Veuillez entrer votre mot de passe pour continuer la suppression du compte.",
        [
          { text: "Annuler", style: "cancel" },
          {
            ...
            onPress: async (password) => {
              ...
              try {
                ...
                showMessage({
                  message: "Ré-authentification échouée",
                  description: "Mot de passe incorrect ou autre erreur.",
                  type: "danger",
                });
              }
            },
    ...
    >
      <Animated.View entering={FadeIn.duration(400)} className="mb-20">
        ...
        <View className="px-4 -mt-16">
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md">
            ...
            <View className="flex-row justify-between items-start">
              <Animated.View entering={FadeInLeft.delay(100).duration(100)}>
                <TouchableOpacity
                  activeOpacity={0.9}
            ...
            <View className="mt-4">
              ...
              >
                {userInfo?.biography || "C'est vide par ici ..."}
              </Text>
              ...
              {/* Badge profil visible ou non */}
              {accountIsVisible ? (
                <View className="inline-flex items-center px-3 py-1 rounded-full bg-green-100 dark:bg-green-900 self-start">
                  <Text
                    style={{ fontFamily: "Inter_500Medium" }}
                    className="text-green-800 dark:text-green-300 text-sm font-semibold"
                  >
                    Profil visible
                  </Text>
                </View>
                ...
                <View className="self-start">
                  <View className="inline-flex items-center px-3 py-1 rounded-full bg-red-100 dark:bg-red-900 mb-1">
                    <Text
                      style={{ fontFamily: "Inter_500Medium" }}
                      className="text-red-800 dark:text-red-300 text-sm font-semibold"
                    >
                      Profil non visible
                    </Text>
                  </View>
        ...
        <View className="px-4 mt-6">
          ...
          {/* {accountIsVisible && <ProfileToggle />} */}
          <ThemeToggle />
          {/* Menu items */}
          <View className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden">
            {/* Section principale */}
            <View className="mt-4 bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm">
              {[
                ...
                .map((item, index, arr) => (
                  <View key={index}>
                    <Pressable
                      ...
                      className="flex-row items-center justify-between px-6 py-4 active:bg-gray-100 dark:active:bg-gray-700"
                    >
                      <View className="flex-row items-center">
                        ...
                        <Text
                          style={{ fontFamily: "Inter_400Regular" }}
                          className="ml-4 text-gray-700 dark:text-gray-300 text-base"
                        >
                          {item.title}
                        </Text>
                      </View>
                    ...
                    {index < arr.length - 1 && (
                      <View className="h-px bg-gray-100 dark:bg-gray-700 mx-6" />
                    )}
                  </View>
                ))}
            </View>
