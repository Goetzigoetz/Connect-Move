...
import { View, ActivityIndicator, Text, TextInput, TouchableOpacity, StyleSheet, Platform, KeyboardAvoidingView } from "react-native";
...
const Chat = ({ navigation, route }) => {
  ...
  const conversationTitle = route?.params?.conversation?.activity?.title || route?.params?.conversation?.title || "Groupe";
  const activityImage = route?.params?.conversation?.activity?.images?.[0] || route?.params?.conversation?.activityImage;
  ...
  useLayoutEffect(() => {
    ...
    // Configurer le header avec l'image de l'activité
    navigation.setOptions({
      headerRight: () => activityImage ? (
  ...
  }, [navigation, setIsTabBarVisible, conversationTitle, activityImage]);
  ...
  useEffect(() => {
    ...
    // Marquer aussi comme lu quand on quitte
    return () => {
      markAsRead();
    };
  }, [conversationId]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        if (!snapshot.empty) {
          // Récupérer tous les senderIds uniques
            ...
            senderIds.map(async (senderId) => {
              try {
                ...
                if (userDoc.exists()) {
                  ...
                  usersData[senderId] = {
                    username: userData.username || userData.displayName || i18n.t("utilisateur"),
          ...
          const messagesData = snapshot.docs.map((docSnap) => {
            ...
            const senderInfo = usersData[data.senderId] || {};
            return {
              _id: docSnap.id,
              text: data.message,
              ...
              user: {
                _id: data.senderId,
                name: senderInfo.username || i18n.t("utilisateur"),
                avatar: senderInfo.avatar || data.senderAvatar || null,
              },
            };
          });
    ...
    async (newMessages = []) => {
      ...
      try {
        ...
        await addDoc(collection(db, "messages"), {
          ...
          senderId: user.uid,
          senderUsername: user?.displayName || i18n.t("utilisateur"),
        ...
        if (conversationData?.participants) {
            ...
            conversationData.participants.map(async (participantId) => {
              if (participantId !== user.uid) {
                ...
                if (data?.expoPushToken) {
                  return { id: participantId, expoPushToken: data.expoPushToken };
                }
              }
              return null;
            })
          ...
          const validParticipants = participantsData.filter(Boolean);

          const notificationMessage = {
            title: conversationTitle,
            desc: `${userData.username || user?.displayName}: ${messageToSend.text.substring(0, 50)}${
              messageToSend.text.length > 50 ? "..." : ""
            }`,
          ...
          for (const participant of validParticipants) {
            try {
              await sendNotifs(participant, notificationMessage);
            } catch (error) {
  ...
  // Fonction pour personnaliser les bulles de messages (avec avatars pour les groupes)
  const renderBubble = (props) => {
    ...
    // Messages des autres utilisateurs avec avatar et nom
    return (
      <View style={styles.bubbleWrapperLeft}>
        ...
        {props.currentMessage.user.avatar ? (
          ...
          <View style={[styles.userAvatar, styles.userAvatarPlaceholder, { backgroundColor: isDarkMode ? "#27272A" : "#E5E7EB" }]}>
            <Ionicons name="person" size={16} color={isDarkMode ? "#71717A" : "#9CA3AF"} />
          </View>
        )}

        <View style={{ flex: 1 }}>
          {/* Nom de l'utilisateur */}
          <Text style={[styles.senderName, { color: isDarkMode ? "#A1A1AA" : "#6B7280" }]}>
  ...
  const renderChatEmpty = () => (
    <View style={[styles.emptyContainer, { transform: [{ scaleY: -1 }] }]}>
      ...
      <Text style={[styles.emptySubtitle, { color: isDarkMode ? "#71717A" : "#64748B" }]}>
        Envoyez votre premier message dans ce groupe 👋
      </Text>
    </View>
  );
  ...
  return (
    ...
    >
      <GiftedChat
        ...
        renderAvatar={() => null} // On gère l'avatar manuellement dans renderBubble
        renderSend={() => null}
...
const styles = StyleSheet.create({
  ...
  userAvatar: {
    ...
    borderRadius: 16,
    marginRight: 8,
    marginTop: 20, // Aligné avec le nom
  },
