...
const handleJoinActivity = async () => {
  try {
    ...
    const pointsDocSnap = await getDocs(pointsDocRef);
    const pointsConfig = pointsDocSnap.data();
    const eventPoints = isWeekend
      ? Number(pointsConfig?.join_weekevent) + Number(pointsConfig?.joint_basic_event_point)
    ...
    <View className="flex-1 bg-white dark:bg-gray-900">
      ...
      <Animated.View entering={FadeInDown.duration(400)} className="p-5">
        ...
        <Pressable
          ...
          style={{ marginTop: 50 }}
          className="bg-teal-500 py-4 rounded items-center"
